...
예를 들어 정수, 실수, 문자(열)이 하나의 리스트로 묶일수도 있다.
'''
...
#           0       1     2      3      4     5       6        7
# fruits = ['사과', '포도', '수박', '참외', '배', '자두', '복숭아', '바나나']
# print(f'fruits: {fruits[6]}')

# 리스트 길이(아이템 개수) 조회
'''
리스트의 길이란 리스트의 아이템 개수를 뜻하는 것으로 len() 함수를 사용하면 알 수 있습니다.
다음은  len() 함수를 이용해서 리스트의 길이를 확인하는 코드입니다.
'''

# numbers = [1,2,3,4,5]
# print(f'nmbers: {numbers}')
# print(f'numbers lenght: {len(numbers)}')
...
# str = 'hellllllllllllllllllllllllllllllllo'
# print(len(str))      # 35글자
...
# balls = ['야구공', '축구공', '탁구공', '골프공', '농구공']
# i = 0
# while i < len(balls):    #balls의 길이가 작을때까지 계속 반복해줘
#     print(f'item: {balls[i]}, index: {i}')
#     i += 1

# quiz) 다음 리스트에서 마지막 인덱스 값을 출력하는 프로그램을 만드시오
# sports = ['baseball',' baketball', 'tennis', 'golf', 'soccer']
# lenVar = len(sports) - 1
# print(sports[lenVar])
...
# tagetIdx = languages.index('python')

# 아이템 기존 리스트에 삽입
# 리스트 마지막에 삽입
sports = ['football','basball','volleyball']
...
# quiz) 취미 추가하기
'''
취미들을 저장할 리스트를 정의하고 사용자가 입력한 취미가 추가되는 프로그램을 만들업자.
그리고 취미의 개수를 출력하자
'''
hobbies = []
...
# 특정 위치에 아이템 삽입
# 리스트의 원하는 위치에 아이템을 삽입할 때는 insert() 함수를 사용한다.
...
리스트를 보고 1~10까지 숫자 중 누락된 숫자를 찾아보자
'''
numbers = [1, 2, 3, 4, 5, 7, 8, 9]
numbers.insert(5,6)
...
sports = ['football', 'baseball', 'volleyball', 'baskeball']
print(f'sports: {sports}')
sports.pop(2)
print(f'sports: {sports}')
...
# quiz) sports list에서 volleyball 을 삭제하는 프로그램을 만들어보자
sports = ['football', 'baseball', 'volleyball', 'baskeball']
