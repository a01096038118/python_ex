...
# 인덱스(index): 아이템에 부여된 식별번호 -> 데이터를 식별하기 위해서 사용
#    0      1      2
# ['사과','포도','복숭아']
...
# 리스트의 길이( 아이템 개수 )
cnt = len(fruits)
print(f'fruits: {cnt}')    # 3

# 리스트의 마지막 아이템 인덱스 값은 리스트의 길이 - 1 이다.
print(f'last data: {fruits[len(fruits)-1]}')

# 리스트의 전체 데이터 조회
# 리스트는 반복 가능한 객체(데이터) --> 이터러블한 데이터
for fruit in fruits: 
    print(f'fruit: {fruit}')
