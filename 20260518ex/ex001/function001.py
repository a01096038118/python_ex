...
# 함수 정의하기
# '''
# 사용자 함수를 만든다는 것은  '함수를 정의한다.' 라고 한다.
# 함수를 정의 할때 def 키워드를 사용한다. 그리고 함수명은 콜론(:), 실행부를 이용한다.
# '''
# '''
# def 함수명(): 
#     실행부(함수 기능)
...
# # quiz) 내 노트북은 몇 인치 일까요?
# '''
# 고등학교 졸업 기념으로 노트북을 하나 장만했습니다.
# 노트북 사이즈에 꼭 맞는 파우치를 하나 구매하려고 하는데 사이즈 표에 인치로만 
# 표시되어 있습니다.
...
# hourData = float(input('이동시간 입력: '))
# speedData = float(input('이동속도 입력: '))
...
# '''
# 출신 국가를 선택하면 해당하는 국가의 인사말이 출력되는 
# 프로그램을 함수를 이용해서 만들어보자.
# 1. 한국       2. USA       3. Japan
# '''
...
# selectedMenuNumber = int(input('Where are you from? 1. 한국    2. USA   3. Japan'))

# if selectedMenuNumber == 1:
#     introKor()
# elif selectedMenuNumber ==2:
#      introEng()
# elif selectedMenuNumber ==3:
#     introJap()
